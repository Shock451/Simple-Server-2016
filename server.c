...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* absolutepath = strchr(line, line[4]);
            if (line[3] != ' ' || (strchr(line, '"') != NULL))
            {  
                error(400);
                continue;
            } 
            ...
            for (n = strlen(absolutepath); i < n && (absolutepath[i] != ' ' && absolutepath[i] != '?'); i++);
            
            char query[n];
            n = 0;
            if (absolutepath[i] == '?' && !isspace(absolutepath[i+1]))
            {
                i+=1;
                for (;!isspace(absolutepath[i]); i++, n++)
                {
                    query[n] = absolutepath[i];
                }
                query[n] = 0;
                i-=1;
            }
            ...
            // version check
            char* version = strchr(absolutepath, ' ') + 1;
            ...
            char* extension = strrchr(absolutepath, '.') + 1;
            if (extension == NULL)
            {
                error(501);
                continue;
            ...
            char altroot[strlen(root)];
            strcpy(altroot, root);
            char* path = strcat(altroot, absolutepath);
